...
def load_data_graph(direction_path, dataset_name, normalization='normalize', load_saved=False,
              k_nearest_neighobrs=50, struct_noise_level = 0.0, feature_noise_level = 0.0):
    print('struct_noise_level : ', struct_noise_level)
        ...
        for i in range(features.shape[1]):
            ...
            if feature_noise_level > 0.0:
                ...
                if dataset_name in dense_datasets:
                    ...
                    #print(f'----------------feature_noise_level: {ratio}-----------------')
            ...
            fea, wave, hat, norm, weight = load_single_view_data(noisy_feature, dataset_name, normalization, i,
                                                                 load_saved, k_nearest_neighobrs, prunning_one,
                                                                 prunning_two, common_neighbors, struct_noise_level)
...
def load_single_view_data(feature, dataset_name, normalization, view_no, load_saved, k_nearest_neighobrs,
                          prunning_one, prunning_two, common_neighbors, struct_noise_level=0.0):
    print('struct_noise_level : ', struct_noise_level)
    ...
    if load_saved is not True:
        ...
        if struct_noise_level > 0.0:
            print(f'-----------------------struct_noise_level: {struct_noise_level}------------------------')
