...
@File   : hach_mvc
@Desc
    Definition of the proposed HACH-MVC framework
'''
...
class HACHMVC(nn.Module):
    ...
    def __init__(self, view_dims, pm_hidden_dims, am_hidden_dims, sm_hidden_dims, num_clusters):
        ...
        #self.graph_decoder_module = nn.ModuleList() 
        for i in range(self.num_views):
            ...
            self.decoder_module.append(MLFPN_FC(list(reversed(temp_dims)), nn.ReLU()))  #Adding and decoding layers, paying attention to the dimensions
            
            # The latent dimension is the output dimension of the GCN encoder.
            #latent_dim = pm_hidden_dims[-1]
            #self.graph_decoder_module.append(BilinearDecoder(latent_dim=latent_dim))

        aemp_dims = []
        aemp_dims.extend(am_hidden_dims)
        self.full_graph_module = MLFPN_GCN(aemp_dims, nn.ReLU())
        # define the fusion module
        self.fusion_module = FusionLayer(num_views=self.num_views, fusion_type='weighted')
        self.fusion_adj = FusionLayer(num_views=self.num_views , fusion_type='weighted')
    ...
    def forward(self, feats, adjs):
        ...
        for i in range(self.num_views):#graph autoencoder
            ...
            #adj_bar = self.graph_decoder_module[i](hidden_pr)
            adj_bar_list.append(adj_bar)
            adj_list.append(adjs[i])

        # computation in the fusion module
        combined_feature_pr = self.fusion_module(hidden_pr_list)#heterogeneous graph of the original view-specific graph

        #construct hegrou graph
        adj_all = all_fg_consstruct(hidden_pr_list, adj_bar_list, self.num_views)
        adj_bar_fusion = self.fusion_adj(adj_bar_list)
        adj_fusion = self.fusion_adj(adj_list)
        ...
        return combined_feature, combined_feature_pr, q, X_bar_list, adj_list ,adj_bar_list , hidden_pr_list , adj_fusion, adj_bar_fusion
